...
async def async_main() -> None:
    parser = argparse.ArgumentParser(description="运行 Core 抽取实验")
    parser.add_argument("--pdf", type=Path, default=DEFAULT_PDF_PATH)
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("experiments/outputs/core_field_extraction"),
    )
    ...
    payload = await run_stage(PROJECT_ROOT, pdf_path, "core")
