...
- 候选必须来自合同正文明确陈述或约定的业务事实；主要书写语言、页数、文件格式、扫描/OCR
  质量等文档载体属性不进入 Attribute 候选，正文明确约定的适用/效力语言除外。
...
字段定义还必须内部自洽：meaning、output 子字段作为正例或组成部分声明的事实，不能又被
extraction_rule 整体排除。同组收敛产生多个兄弟字段时，程序补入的 `not_meaning` 是硬边界；
例如付款方式不能再次吸收分期付款安排，付款安排也不能把预付款或首付款一概排除。第二阶段
回扫会进一步拒绝把多阶段比例、金额和期限的整段付款计划当成付款方式，并只局部重试当前
合同—字段任务。

封闭 enum 必须覆盖其 extraction_rule 明示的全部字面类别；如果规则同时允许专用发票和普通
发票，`values` 只提供专用发票就不能准入，否则强 Schema 会把普通票强制误映射成专票。发票
回扫还会核对专票/普票原文与规范值一致，并要求税率和备注保留直接开票语境。结构化候选已经
包含的子字段查询切片不再保留为兄弟字段，例如预付款比例应由付款阶段数组按阶段查询，而不是
重复成为一个顶层 Attribute。
...
失败允许其余字段继续执行；单字段初次调用和一次纠错均失败后会被省略，成功字段继续输出，
工作流在处理元数据中记录局部失败状态、字段 ID 和脱敏错误，禁止把技术失败转换为
`not_found`。字段级动态 Schema 和业务校验仍然保持不变，不允许把未通过校验的字段混入
合同候选。
...
Attribute 的非空率不是完整性指标。每个成功完成提取的目录字段都必须得到一个合法终态：
...
因此，字段质量门禁检查的是“成功输出的字段是否有合法终态、所有采用值是否有证据、值是否
满足目录 Schema”，而不是要求大部分字段非空。`not_found`、`not_applicable`、`ambiguous`
和 `conflicting` 都必须令规范值为 `null`，但可以保留最小相关原文用于审核。重试后仍没有形成
合法终态的字段属于技术失败，不进入 Attribute 业务结果，并由 `processing.attribute_extraction`
单独列出。
...
伪装成 10 个字段已经完成提取。若活动目录的全部字段均在一次纠错后失败，业务结果也可能为空，
但处理状态必须是 `completed_with_failures` 并列出全部跳过字段；只有目录显式为
`status: empty` 且 `fields: []` 时，生产图才会跳过 Attribute 节点并返回无失败诊断的空列表。
...
正式 `FieldDiscoveryService` 已接入默认算法。CLI 选择 `discovery` 时会先按独立目录提取固定
Core/Attribute，再执行候选级准入、批次内向量归并、组级收敛和冻结字段全合同集回扫。
候选结果只形成专家审核材料，不会自动写入本文件描述的生产 Attribute 目录。

正式 discovery 批次会把冻结候选定义和回扫统计保存到
`data/definitions/discovery/result/<batch_id>.yaml`。其中字段定义与本页 Attribute 契约同形，并仅
增加批次相关的 `statistics`；该文件仍是 `status: draft` 的审核材料。

> **当前限制：** discovery 尚未实现专家晋级与目录版本治理；不得把批次结果 YAML 直接当作生产字段，或通过放宽字段契约伪造审核完成。
