...
GROUP_REFINEMENT_SYSTEM_MESSAGE = render_discovery_prompt(
    "00d_group_refinement_system.txt", {}
)
GLOBAL_GATE_SYSTEM_MESSAGE = render_discovery_prompt(
    "00e_global_gate_system.txt", {}
)
LogEmitter = Callable[[str], Awaitable[None]]
SchemaModel = TypeVar("SchemaModel", bound=BaseModel)


def _drop_invalid_optional_patterns(
    output: OutputDescription,
) -> tuple[OutputDescription, list[str]]:
    """只移除无法编译的可选 pattern，其他输出结构和约束保持不变。"""
    ...
    def visit(node: Any, *, path: str) -> None:
        ...
        if isinstance(pattern, str):
            try:
                ...
                node.pop("pattern")
                removed_paths.append(f"{path}.pattern")
        properties = node.get("properties")
        if isinstance(properties, list):
            for property_item in properties:
                if isinstance(property_item, dict):
                    visit(
                        property_item.get("output"),
                        path=f"{path}.properties.{property_item.get('field_id', '?')}.output",
                    )
        items = node.get("items")
        if isinstance(items, dict):
            visit(items.get("output"), path=f"{path}.items.output")

    visit(payload, path="output")
    return OutputDescription.model_validate(payload), removed_paths
...
async def _define_final_field(
    *,
    ...
    attempts: list[dict[str, Any]] = []
    correction: str | None = None
    last_response: FinalFieldDefinitionSuggestion | None = None
    ...
    for attempt in range(max_validation_retries + 1):
        ...
        try:
            ...
            if attempt >= max_validation_retries and last_response is not None:
                recovered_output, removed_paths = _drop_invalid_optional_patterns(
                    last_response.output
                )
                if removed_paths:
                    try:
                        recovered = validate_final_field_definition(
                            suggestion=last_response.model_copy(
                                update={"output": recovered_output}
                            ),
                            plan=plan,
                            sibling_plans=sibling_plans,
                            profile=profile,
                        ...
                        attempts.append(
                            {
                                "attempt": attempt + 2,
                                "status": "pattern_fallback_rejected",
                        ...
                        return recovered.report(), {
                            ...
                            "status": "succeeded",
                            "recovery": {
                                "mode": "drop_invalid_optional_patterns",
...
async def refine_group(
    *,
    ...
    if failed_definitions:
        return {
            "group_id": profile.group_id,
            "status": "partially_succeeded" if final_fields else "failed",
    ...
    *,
    ...
    try:
        ...
        for report in reports:
            if report.get("status") in {"succeeded", "partially_succeeded"}:
    ...
    return {
        ...
        "partially_succeeded_group_count": sum(
            item.get("status") == "partially_succeeded" for item in reports
        ),
