"""字段发现模型使用的递归类型描述，以及到正式字段 output 的确定性编译器。"""
...
- format 可用于说明任一类型的规范化格式；string/date 仅在正则能表达真实约束时补充 pattern，
  禁止使用 `.*`、`^.*$`、`.+` 或 `^.+$` 等无约束正则，也不得用多个中文业务短语的正则分支
  伪装枚举；封闭集合应使用 enum，开放文本应省略 pattern；number/integer 可补充 unit、minimum、maximum。
...
def compile_output_description(
    ...
    if description.pattern:
        compact_pattern = re.sub(r"\s+", "", description.pattern)
        if compact_pattern in {".*", "^.*$", ".+", "^.+$"}:
            raise ValueError(
                f"{path}.pattern 不得使用 {description.pattern!r} 这类无约束正则；"
                "没有真实格式限制时必须省略 pattern。"
            )
        # pattern 只能描述字符形态，不能把若干自然语言类别藏进 string 正则。
        # 允许“数字+年/月/天”等每个分支都含真实正则结构的词法格式。
        unwrapped = compact_pattern.removeprefix("^").removesuffix("$")
        if unwrapped.startswith("(") and unwrapped.endswith(")"):
            unwrapped = unwrapped[1:-1]
        branches = unwrapped.split("|")
        literal_business_branches = [
            branch
            for branch in branches
            if re.search(r"[\u4e00-\u9fff]", branch)
            and not re.search(r"[\[\]().+*?{}\\]", branch)
        ]
        if (
            description.type == "string"
            and len(branches) >= 3
            and len(literal_business_branches) >= 2
        ):
            raise ValueError(
                f"{path}.pattern 不得用多个自然语言业务短语伪装枚举；"
                "封闭集合请使用 enum，开放文本请省略 pattern。"
            )
        try:
            re.compile(description.pattern)
